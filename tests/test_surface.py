...
class SurfaceTests(unittest.TestCase):
 def run_surface(self,*a,env=None):return subprocess.run(["python3",str(SURFACE),*a],check=True,text=True,capture_output=True,env=env)
 def test_register_and_suggest_cross_surface(self):
  with tempfile.TemporaryDirectory() as td:
   e=os.environ.copy();e["AGENT_DISPATCH_HOME"]=td
   for m,r,t in (("luna","high","cheap"),("sol","low","frontier"),("terra","medium","general")):self.run_surface("register-cell","--model",m,"--effort",r,"--tier",t,env=e)
   x=json.loads(self.run_surface("suggest","--task-class","bounded_implementation","--current-model","terra","--current-effort","medium",env=e).stdout)
 ...
 def test_promote_project_route(self):
  with tempfile.TemporaryDirectory() as td,tempfile.TemporaryDirectory() as repo:
   ...
   x=json.loads(self.run_surface("promote","--repo-root",repo,"--project","demo","--task-class","bounded_implementation","--domain","rust",env=e).stdout)
